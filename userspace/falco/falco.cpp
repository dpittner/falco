...
falco_engine *engine_to_swap = nullptr;
		    ...
		    int &result)
{
	...
	while(1)
	{
		...
		// the outputs.
		bool engine_cmp_res = e.compare_exchange_strong(h, engine_to_swap);
...
{
	// auto t = reinterpret_cast<std::tuple<falco_engine *, falco_engine *> *>(engine);
	falco_engine *bluengine = reinterpret_cast<falco_engine *>(engine);
	// falco_engine *e = std::get<1>(*t);

	engine_to_swap = bluengine->clone();
	engine_to_swap->load_rules(rules_content, false, true);
...
int falco_init(int argc, char **argv)
{
	...
	try
	{
		...
		// falco_engine *engine_to_swap = nullptr;
		// std::tuple<falco_engine *, falco_engine *> a = std::make_tuple(bluengine, engine_to_swap);
